---
description: 此处介绍了Lights-IM的服务端验证算法
---
...
服务端和客户端的通讯识别为一个加密的组合字符串

组合字符串的格式为 **APIKey+当前时间戳 \(H:i:s\)+SercetKey**

例如，**假设**APIKey=asd45da5s64sdzcfv6szdf6 并且**假设** SercetKey=fz5r2g4z5d4f5zs45g4zesr5

当前时间戳假设为 **00:23:56** \(凌晨零点23分56秒\)

那么组合字符串就是 **asd45da5s64sdzcfv6szdf600:23:56fz5r2g4z5d4f5zs45g4zesr5**

再使用**MD5\(32位小写\)**加密 , 最终的加密组合字符串就是  **e4c69f1c5b5a800d89c9ca71539db690**
...
$timecuo = date("H:i:s",strtotime("+0 second"));
$STDString = $APIKey.$timecuo.$SerceyKey;
echo md5($STDString);
?>
```
...
{ 
  ...
  char szDigest[16];  
  char encrypt[200];
  string p1="<Your APIKey>";
  string p2="<Your SercetKey>";
  int opt=-1,optt=-1;
  int len=p1.size()+8+p2.size();
  for(register int i=0;i<len;i++)  {
    if(i<=p1.size()) encrypt[i]=p1[i];
    if(i>=p1.size() && i<=p1.size()+8) encrypt[i]=date.tmp1[++opt];
    if(i>=p1.size()+8) encrypt[i]=p2[++optt];
  }
...
此部分由php解决，将加密后的字符串传至php内进行验证

由for枚举规定范围内的时间戳，随后进行枚举验证

通过验证后将访问ip添加进入防火墙内并记录访问时间

当重复的ip多次访问会自动刷新时间，避免文件沉积

此时访问ip就获得了有效时限内的对服务器的控制权
